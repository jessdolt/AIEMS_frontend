const inputUtil = {
  init: () => {
    ...
    //manage
    const editBtnManageAcc = document.querySelector(
      ".managePage .btn-con .edit"
    );
    const cancelBtnManageAcc = document.querySelector(
      ".managePage .btn-con .cancel"
    );
    const cardCon = document.querySelector(".managePage .card-con");
    const inputs = document.querySelectorAll(
      ".managePage .card-con #edit-form input"
    );
    //addInfo
    ...
    if (codeTextBox) {
      codeTextBox.addEventListener("input", (e) => {
        ...
        if (codeTexField.value.length > codeTexField.maxLength - 1) {
          codeTexField.value = codeTexField.value.slice(
            0,
            codeTexField.maxLength
          );
    ...
    function setStudentId(e) {
      try {
        let selectedBatch =
          e.target.selectedOptions[0].attributes["data-batch"].value;
    ...
    function setInputFields(e) {
      ...
      if (currentStat.value == "Student" || currentStat.value == "Unemployed") {
        allInput.forEach((element) => {
          if (element.id == "grad-date" || element.id == "first-emp-date") {
